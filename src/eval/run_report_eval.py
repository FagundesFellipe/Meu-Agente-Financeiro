...
from eval.evaluate_report_query_adaptor import evaluate_report_query_extraction
from eval.json_correctness_metric import (
    build_report_correctness_metrics,
    build_report_json_correctness_metric,
)

warnings.filterwarnings("ignore")

DATASET = Path(__file__).resolve().parent / "dataset/golden_report.json"
...
async def build_tests_cases(golden_dataset: list[Golden]) -> list[LLMTestCase]:
    ...
    for record in golden_dataset:
        actual_output = await evaluate_report_query_extraction(record.input)
        tests.append(
            LLMTestCase(
                name=record.name,
                input=record.input,
...
async def main() -> None:
    ...
    metric_json = build_report_json_correctness_metric()
    metric_geval = build_report_correctness_metrics()
